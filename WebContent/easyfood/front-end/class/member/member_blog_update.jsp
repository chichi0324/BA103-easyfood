         ...
         <table class="table table-hover table-bordered table-condensed">
          ...
          <tbody>
            <tr>
              <td>標題</td>
              <td>
                <input type="text" class="form-control input_date" name="bl_name" id="title"  placeholder="請輸入標題..." 
                 value='<%= blogVO.getBl_name() %>'/>
              </td>
...
<!--             <tr>
            ...
            <tr>
              <td>分享<br>店家</td>
              <td>
             <select style="width: 200px;"  name="article_store">
                <option value="請選擇">請選擇</option>
                <c:forEach var="strVO" items="${strList}">
                   <option value="${strVO.str_no}" ${(strVO.str_no==blogVO.str_no)? 'selected':''} >${strVO.str_name}</option>
                </c:forEach>
              </select> 
              　                    (選擇想分享的店家,可設定店家連結)
              </td>
            ...
            <tr>
              <td>日記<br>編輯</td>
              <td>
                <textarea name="bl_con" id="content" rows="10" cols="80"
                 ><%= blogVO.getBl_con() %></textarea>
                <script>
             ...
             <input type="hidden" name="bl_no" value="<%=blogVO.getBl_no()%>">
             <input type="hidden" name="action" value="update">
         <div class="text-center">
            <input type = 'submit' value = '修改' class="btn btn-success  btn login-button" onclick = 'processData()'>
         </div>
        </form>
        </div>
