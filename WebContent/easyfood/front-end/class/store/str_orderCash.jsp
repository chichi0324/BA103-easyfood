...
<% 
	int count = 0;
/* 	String startDate = null, confirmDate = null, etdShip = null; */
%>
				...
				<div class="page-header order-page">訂單總覽</div>
					...
					<form action="<%=request.getContextPath() %>/ord/ord.do" method="post">
			     	...
			     	<table class="table">
				    	...
				    	<div style="text-align:end;" id="bill">
				    		<form action="<%=request.getContextPath() %>/ord/ord.do" method="post">
				    			<select name="ord_status" class="select-sm">
								<option value="已付款">已付款</option>
								<option value="未取餐">未取餐</option>
							</select>
							<button class="btn btn-mydish btn-sm" style="font-size:initial;">結案</button>
				    			<input type="hidden" name="ord_no" value="${ordVO.ord_no}">
				    			<input type="hidden" name="action" value="CASH_CHECK">
				    		</form>
				    	</div>
