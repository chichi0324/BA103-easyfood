...
<body>
					...
					<form action="<%=request.getContextPath() %>/ord/ord.do" method="post">
			     	...
			     	<table class="table">
				    	...
				    	<div style="text-align:end;">
				    		<form action="<%=request.getContextPath() %>/ord/ord.do" method="post">
				    			<button class="btn btn-mydish btn-sm" style="font-size:initial;" name="action" value="SHIP_ORDERS" id="btn_ship">出餐</button>
	...
	<script>
		...
		if(status == "已取消") {
			var btn = document.getElementById("btn_ship");
			btn.remove();
		}
		var status = document.getElementById("status").textContent;
		if(status == "已出貨") {
