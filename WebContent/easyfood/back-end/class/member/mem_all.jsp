...
List<MemVO> memAllList = (List<MemVO>) memSvc.getAll();

pageContext.setAttribute("memAllList", memAllList);
%>
<!DOCTYPE html>
<html lang="">
<head>
...
<body>
<%if(memAllList.size()==0){ %>
		<br>
		<div style="color: red" class="text-center">目前無資料</div>
<%}else{%>
 ...
 <form action="<%=request.getContextPath()%>/easyfood/back-end/class/member/memVio.do" method="get">
	<table class="table table-striped table-hover"
		...
		<tbody>
		<c:forEach var="memVO" items="${memAllList}" varStatus="s">
			<tr>
				<td>${memVO.mem_acc}</td>
	...
	<div align="center">
		<input type='hidden' value="<%=memAllList.size() %>" name="number">
		<input type='hidden' value="noData" name="mem_vio">
		<input type='hidden' value="noData" name="mem_stas">
